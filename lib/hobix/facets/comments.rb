      ...
      def get app
        if app.respond_to? :action_uri
          ...
          when "comment"
            ...
            # Another quick hack for sending email notifications of new
            # comments.
            if @weblog.requires.detect { |i| i['hobix/plugin/comment-notification'] }
              CommentNotification.notify(@weblog, entry_id, comment)
            end
